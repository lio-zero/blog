...
它是**自我描述的**并且易于理解，同时也易于机器解析和生成，并有效地提升网络传输效率。
...
JavaScript 提供了两个方法，用于处理 JSON 数据格式，如下：
...
console.log(json ; // "{"name": "lisi","age": 18}"
```
...
- 字符串使用双引号。JSON 中没有单引号或反引号。所以 `'lisi'` 被转换为 `"lisi"`。
- 对象属性名称也是双引号的。这是强制性的。所以 `age: 18` 被转换成 `"age": 18`。
...
{ "name": "lisi", "age": undefined } // 不能使用 undefined
