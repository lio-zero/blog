...
[History API](https://developer.mozilla.org/en-US/docs/Web/API/History_API) 允许你与浏览器历史交互，触发浏览器导航方法并更改地址栏内容。
...
IE 从版本 10 开始支持它，如果你需要支持 IE9 及更早版本，请使用 [history.js](https://github.com/browserstate/history.js/) 库。

## 访问 History API

History API 在 `window` 对象上可用，因此你可以这样调用它：`window.history` 或 `history`，因为 `window` 它是全局对象（可以省略）。
...
- 第二个参数目前未被主要浏览器使用（[查看](https://github.com/whatwg/html/issues/2174)），因此通常传递一个空字符串。
...
其返回当前状态对象（传递给 `pushState` 或 `replaceState` 的第一个参数）。
...
只有在做出浏览器动作时，才会触发该事件。例如：用户点击浏览器的前进、后退按钮，或者在 JavaScript 代码中调用 `history.go()`、`history.back()` 或 `history.forward()` 方法。
