...
一个常见的场景是：你注册了一个服务，并从服务仪表板下载了一个 JWT。从现在开始，你将使用它对服务器的所有请求进行身份验证。

另一个相反的用例是，当你管理 API 并且客户端连接到你时发送 JWT，并且你希望你的用户仅通过传递令牌来发送后续请求。
...
- 在跨站脚本（XSS）攻击中，恶意 JavaScript 被注入到你的网站并运行。此代码可以从 `localStorage` 或 Cookie 中获取身份验证令牌，或将其发送给第三方。
- 在跨站请求伪造（CSRF）攻击中，HTTP 请求从第三方网站发送到你的网站（通常通过在恶意网站上提交隐藏表单）。如果用户已经登录到你的网站，他们的身份验证 Cookie 将自动发送，在他们不知情的情况下代表他们发起操作。
...
然而，JavaScript 也不能用于设置带有 `HttpOnly` 标志的 Cookie。Cookie 只能由服务器设置。如果你的应用程序完全是客户端的，并且你使用 JavaScript 存储令牌，则不能使用此标志。在这种情况下，Cookie 与 `localStorage` 一样容易受到 XSS 攻击。

**那么，你应该使用哪一个？**

- 对于仅限客户端的应用程序，`localStorage` 实际上可能更安全，因为它不易受到 CSRF 攻击。
- 如果你可以使用服务器端代码获取和设置令牌，那么带有 `HttpOnly` 标志的 Cookie 可能是更好的选择。

> 推荐：[HTTP Cookie](https://github.com/lio-zero/blog/blob/main/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C/HTTP%20Cookie.md)

## 选择最好的 JWT 库

根据你使用的语言和环境，你可以从多个库中进行选择。最受欢迎的是 [jwt.io](https://jwt.io/#libraries-io) 网站。
...
你不应该将 JWT 用于会话。使用常规的服务器端会话机制，因为它效率更高，更不容易暴露数据。
...
- [前端无痛刷新 Token](https://juejin.cn/post/7075348765162340383)
